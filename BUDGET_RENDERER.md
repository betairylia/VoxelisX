...
A fixed-cost alternative to the path-traced feature. Where `VoxelisXRendererFeature` spends an
...
drive `VoxelisXCameraHistory` and both publish into `VoxelisXFrameResources`, so running them
together would have each overwrite the other's frame. (`VoxelisXCameraHistory.BeginFrame` /
...
| File | Role |
| --- | --- |
| `Runtime/Rendering/RendererFeature/VoxelisXBudgetRendererFeature.cs` | settings surface + wiring |
| `Runtime/Rendering/RendererFeature/VoxelisXBudgetSettings.cs` | `VoxelisXBudgetTraceSettings`, `VoxelisXBudgetShadingSettings` |
| `Runtime/Rendering/RendererFeature/Passes/VoxelisXBudgetGBufferPass.cs` | stage 1, the DXR dispatch; also hosts `VoxelisXBudgetLighting` (shared light/sky resolution) |
| `Runtime/Rendering/RendererFeature/Passes/VoxelisXBudgetDenoisePass.cs` | stage 2, filter + deferred shade |
| `Runtime/Rendering/Shaders/Budget/VoxelisXBudget.raytrace` | the raygen |
| `Runtime/Rendering/Shaders/Budget/VoxelisXBudgetShade.shader` | 4 passes: a-trous, temporal, deferred shade, cross resolve |

Reused unchanged: `VoxelisXPresentPass`, `VoxelisXCameraHistory`, `VoxelisXFrameResources`,
`VoxelisXATrousFilterSettings`, `VoxelisXTemporalRadianceSettings`, `PostFlip.shader`,
...
| Include | Contents |
| --- | --- |
| `VoxelisXDenoiseCommon.hlsl` | `_VoxelisXFrameSize`, the shared G-buffer guide textures, coord/normal/luma/hash helpers |
| `VoxelisXATrous.hlsl` | `VoxelisXATrousFilter(coord)` — SVGF-lite edge-aware wavelet |
| `VoxelisXTemporal.hlsl` | `VoxelisXTemporalAccumulate(coord)` — validated bilinear history reprojection |
| `VoxelisXCrossResolve.hlsl` | `VoxelisXCrossResolve(coord)` — delta checkerboard averaging |

The signal each kernel operates on is named by a macro the including shader sets:
`VOXELISX_ATROUS_SIGNAL_TEX` and `VOXELISX_TEMPORAL_SIGNAL_TEX`. Both default to
...
| --- | --- | --- |
| `VoxelisX_outDeterministicRadiance` | `ARGBFloat` | `RGB`: emission (or sky, for a delta ray that missed), pre-multiplied by chain throughput. `A`: 1 surface / 0 direct miss — the present stage clips on it so URP's own skybox shows through |
| `VoxelisX_outAlbedo` | `ARGBHalf` | `RGB`: surface albedo × chain throughput. `A`: unused |
| `VoxelisX_outNormal` | `ARGBHalf` | `RG`: oct-packed world normal in `[0,1]`, `B`: voxel face hash, `A`: linear roughness |
| `VoxelisX_outBudgetSurface` | `ARGBHalf` | `RG`: oct-packed direction *from surface towards viewer*, `B`: metallic, `A`: 1 surface / 0 sky |
| `VoxelisX_outBudgetAOShadowRaw` | `ARGBHalf` | `R`: AO, `G`: sun visibility, `B`: unused, `A`: validity |
| `VoxelisX_outDepth` | `RFloat` | `R`: linear view depth of the **primary** hit |
| `VoxelisX_outMotionVector` | `ARGBFloat` | `XY`: previousUV − currentUV, `Z`: viewZprev − viewZ (NRD 2.5D), `A`: delta-checkerboard flag |
| `VoxelisX_HistoryDepth_*` | `RFloat` | `R`: linear view depth of the **PSR virtual** hit |
| `VoxelisX_HistoryNormal_*` | `ARGBHalf` | `RG`: oct normal, `B`: this surface's expected previous-frame depth, `A`: valid |
...
`VoxelisX_outBudgetSurface` exists purely because of PSR — the shaded surface is not on the camera
...
- `VoxelisXBrickTrace.hlsl:273` terminates on `GetFaceBits(blockID) & normalFlags`, where
...
`VoxelisXBudgetShade.shader`, pass `DeferredShade`, one evaluation per pixel:
...
1. `VoxelisX Budget DXR Trace` — G-buffer + raw AO/shadow.
2. `VoxelisX Budget AO A-Trous Filter 1..N` — shared a-trous over AO/shadow, ping-ponged, rebinding
   `_BudgetAOShadowTex` after each iteration.
3. `VoxelisX Budget AO Temporal Accumulation` — shared temporal kernel into
   `VoxelisXCameraHistory`'s double buffer (signal-agnostic `ARGBHalf`, so AO reuses the targets
   the path tracer fills with radiance).
4. `VoxelisX Budget Deferred Shade` — the lighting model above.
5. `VoxelisX Budget Delta Checkerboard Resolve` — shared cross resolve, keyed off `MotionVector.a`.
6. `VoxelisX Copy To Camera` — `VoxelisXPresentPass`, unchanged.

## Setup

On the renderer asset, add **VoxelisX Budget Renderer Feature** and disable the path-traced
`VoxelisX Renderer Feature`. Assign:

- **Tracer** → `VoxelisXBudget.raytrace`
- **Budget Shade Shader** → `Hidden/VoxelisX/BudgetShade`
...
`VoxelisXDebugView` gained four budget entries (11–14): `BudgetAOShadowRaw`,
