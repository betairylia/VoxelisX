...
This file is an implementation plan only. Treat `Caelix/` as the package to implement in later work; this revision changes only the root design document.

## Current Baseline

Before implementing this plan, the relevant existing engine shape is:

- `CaelixWorld.BrickInfo` contains `EntityId`, `SectorPos`, `BrickOrigin`, `Sector`, and `Neighbors`.
...
public struct VoxelNeighborhoodReader
{
    public static VoxelNeighborhoodReader Create(
        CaelixWorld.BrickInfo brickInfo,
...
public struct AutomataReadContext
{
    ...
    public VoxelNeighborhoodReader CreateReader(CaelixWorld.BrickInfo brickInfo)
    {
...
This example is included so a worker implementing only inside `Caelix/` can understand the desired API usage, even though the sample gameplay file may live outside the package.
...
{
    public NativeArray<CaelixWorld.BrickInfo> brickInfos;
    [ReadOnly] public AutomataReadContext ReadContext;

    public void Execute(int index)
    {
        CaelixWorld.BrickInfo brickInfo = brickInfos[index];
...
```csharp
public bool Execute(
    CaelixWorld.AutomataStageInputs inputs,
...
Integrate into `CaelixWorld.Tick()` before scheduling automata:
