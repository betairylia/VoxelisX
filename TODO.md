...
- Fix queued sector removal in `CaelixRenderer`.
  `CaelixRenderer.Tick` calls `SectorRenderer.RemoveMe` for queued sector removals, but `RemoveMe` only removes the RTAS instance when `shouldRemove` was previously set by `MarkRemove`. The removal path should either mark before removing or make `RemoveMe` unconditional for queued removals.
