...
/// <summary>
/// URP renderer feature for VoxelisX budget mode: a fixed, small ray budget per pixel instead of a
/// path trace.
/// </summary>
/// <remarks>
/// The sibling of <see cref="VoxelisXRendererFeature"/>. Enable exactly one of the two on a
/// renderer asset — they both drive <see cref="VoxelisXCameraHistory"/> and both write
/// <see cref="VoxelisXFrameResources"/>, so running them together would have each overwrite the
/// other's frame.
/// <para>
...
/// </para>
/// <para>
/// The stages hand off through <see cref="VoxelisXFrameResources"/>:
/// <list type="number">
/// <item><see cref="VoxelisXBudgetGBufferPass"/> — DXR trace producing the G-buffer and raw AO/shadow;</item>
/// <item><see cref="VoxelisXBudgetDenoisePass"/> — spatial filter, temporal accumulation, deferred shade;</item>
/// <item><see cref="VoxelisXPresentPass"/> — reused unchanged: debug view selection and copy to the camera target.</item>
/// </list>
...
/// </para>
/// </remarks>
public class VoxelisXBudgetRendererFeature : ScriptableRendererFeature
{
    /// <summary>Budget mode ray tracing shader (VoxelisXBudget.raytrace).</summary>
    [SerializeField] private RayTracingShader tracer;

    /// <summary>Denoise + deferred shade shader (VoxelisXBudgetShade.shader).</summary>
    ...
    [Header("Ray Budget")]
    [SerializeField] private VoxelisXBudgetTraceSettings budgetTrace = VoxelisXBudgetTraceSettings.Default;

    [Header("Shading")]
    [SerializeField] private VoxelisXBudgetShadingSettings budgetShading = VoxelisXBudgetShadingSettings.Default;
    ...
    [SerializeField] private VoxelisXATrousFilterSettings aTrous = VoxelisXATrousFilterSettings.Default;
    ...
    [Header("Debug")]
    [SerializeField, Tooltip("Which VoxelisX buffer to display. Debug views reuse buffers the frame already produced, so they cost nothing extra to render. The stochastic views have no budget-mode equivalent and fall back to the shaded image.")]
    private VoxelisXDebugView debugView = VoxelisXDebugView.Regular;

    private VoxelisXBudgetGBufferPass gbufferPass;
    private VoxelisXBudgetDenoisePass denoisePass;
    ...
    public override void Create()
    {
        ...
        gbufferPass = new VoxelisXBudgetGBufferPass
        {
            renderPassEvent = RenderPassEvent.AfterRenderingOpaques
        };
        denoisePass = new VoxelisXBudgetDenoisePass
        {
            renderPassEvent = RenderPassEvent.AfterRenderingOpaques
        };
    ...
    private VoxelisXBudgetTraceSettings BuildTraceSettings()
    {
        VoxelisXBudgetTraceSettings settings = budgetTrace;
