# VoxelisX: ECS vs Non-ECS Architecture Decision
...
After extensive research and profiling, the decision has been made to **discontinue the VoxelisX-ECS implementation** and return to the proven MonoBehaviour-based architecture with Unity's Job System and Burst compilation.
...
**Proven working implementation**: Existing non-ECS VoxelisX already demonstrates:
...
#### Bandwidth Analysis for VoxelisX
...
❌ **VoxelisX Does NOT Fit ECS Strengths**:

| ECS Strength | VoxelisX Reality |
|--------------|------------------|
...
VoxelisX has inherent hierarchy:
```
VoxelObject
...
Unity ECS is a **specialized tool** optimized for specific use cases. VoxelisX does not match those use cases:

| ECS Optimized For | VoxelisX Requirements |
|-------------------|----------------------|
...
- **Voxel engines with few objects and custom networking** ← VoxelisX
...
The non-ECS VoxelisX already demonstrated superior performance, cleaner parallelization, and simpler code. It's time to leverage that foundation and focus energy on building gameplay features rather than fighting framework limitations.
